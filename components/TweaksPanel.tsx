...
function useTweaks<T extends object>(defaults: T): [T, (keyOrEdits: keyof T | Partial<T>, val?: unknown) => void] {
...
function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const segLimit = ({ 2: 16, 3: 10 } as Record<number, number>)[options.length];
  const fitsAsSegments = segLimit != null && maxLen <= segLimit;
  if (!fitsAsSegments) {
