// Onboarding — 3 welcome slides + initial setup (venit + categorii)

function Onboarding({ onFinish }) {
  const [step, setStep] = React.useState(0); // 0,1,2 welcome; 3 income; 4 categories; 5 done
  const [income, setIncome] = React.useState('');
  const [selected, setSelected] = React.useState(new Set(['food','trans','util','fun','cafe']));
  const [budgetTemplate, setBudgetTemplate] = React.useState('50-30-20'); // 50-30-20 | conservator | custom
  ...
  const cats = window.MF_CATEGORIES.filter(c => c.id !== 'salary' && c.id !== 'free');
  ...
  // Step 4: Categories + Budget template
  if (step === 4) {
    const toggle = (id) => {
                ...
                <button key={t.id} onClick={() => setBudgetTemplate(t.id)} style={{
                  ...
                  {t.needs && (
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      <div style={{ height: 6, borderRadius: 999, width: t.wants * 60, background: '#EA580C' }}/>
                      <div style={{ height: 6, borderRadius: 999, width: t.save * 60, background: '#16A34A' }}/>
                    </div>
                  )}
                </button>
              );
          ...
          {tpl.needs && (
            <div style={{
              ...
              {[
                ...
                { label: 'Plăceri',       color: '#EA580C', v: tpl.wants },
                { label: 'Economii',      color: '#16A34A', v: tpl.save },
              ].map((r, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 10, height: 10, borderRadius: 3, background: r.color }}/>
                    <div style={{ fontSize: 13, color: '#1A3C5E', fontWeight: 600 }}>{r.label}</div>
                    <div style={{ fontSize: 11, color: '#94A3B8' }}>{Math.round(r.v * 100)}%</div>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: r.color, letterSpacing: -0.2 }}>
                    {Math.round(incomeN * r.v).toLocaleString('ro-RO')} lei
                  </div>
                </div>
              ))}
            </div>
